...
passport.use(new LinkedInStrategy({
    ...
    if (req.user) {
        User_1.default.findOne({ linkedin: profile.id }, (err, existingUser) => {
            if (err) {
                return done(err);
            }
            if (existingUser) {
        ...
        User_1.default.findOne({ linkedin: profile.id }, (err, existingUser) => {
            ...
            User_1.default.findOne({ email: profile._json.emailAddress }, (err, existingEmailUser) => {
                if (err) {
                    return done(err);
                }
                if (existingEmailUser) {
...
passport.use(new InstagramStrategy({
    ...
    if (req.user) {
        User_1.default.findOne({ instagram: profile.id }, (err, existingUser) => {
            if (err) {
                return done(err);
            }
            if (existingUser) {
...
passport.use(new OpenIDStrategy({
    ...
    User_1.default.findOne({ steam: steamId }, (err, existingUser) => {
        if (err) {
            return done(err);
        }
        if (existingUser)
